...
const MarketTab = ({ baseAsset, quoteAsset, leverageMode, leverage }) => {
  const dispatch = useDispatch();
  const [placeOrder, { isLoading: isPlacingOrder }] = useFeaturePlaceOrderMutation();
  ...
  const tpValue = Form.useWatch('tpTriggerPx', form);
  const slValue = Form.useWatch('slTriggerPx', form);
  const tpValueSell = Form.useWatch('tpTriggerPx', sellForm);
  const slValueSell = Form.useWatch('slTriggerPx', sellForm);
  ...
  useEffect(() => {
    dispatch(setSelectedTab("market"));
  }, []);
  useEffect(() => {
  ...
  const handleSubmit = async (values) => {
    console.log("values--------perp---------------", values);
    try {
      setSide("buy");
      let profitLossFields = {};
      if (values?.tpTriggerPx) {
        profitLossFields = {
          stopPrice: values?.tpTriggerPx,
          type: "TAKE_PROFIT_MARKET",
        };
      }
      else if (values?.slTriggerPx) {
        profitLossFields = {
          stopPrice: values?.slTriggerPx,
          type: "STOP_MARKET",
        };
      } else {
        profitLossFields["type"] = "MARKET";
      };
  ...
  useEffect(() => {
    ...
    if (buyCurrency === "Crypto") {
      ...
      setMaxBuyValue(Number(availableBal?.[0]?.free) * Number(leverage));
    } else {
    ...
    if (sellCurrency === "Crypto") {
      ...
      setMaxSellValue(Number(availableBal?.[0]?.free) * Number(leverage));
    } else {
      ...
      >
        <Form.Item name="price" validateTrigger="onChange">
          <Input
            ...
            className="input"
            disabled
            placeholder={coinPairCurrentPrice?.price}
            suffix={
              <>
                ...
                rules={!tpValue ? [
                  ...
                  },
                  {
                    validator: (_, value) => {
                      ...
                      // else if (coinPairCurrentPrice?.price < value) {
                      //   return Promise.reject(
