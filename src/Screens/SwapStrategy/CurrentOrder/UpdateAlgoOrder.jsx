...
import { useAmendAlgoOrderMutation } from "../../../Services/Transaction";
import {
...
const UpdateAlgoOrder = ({ baseAsset, quoteAsset, row, setIsModalOpen }) => {
  const theme = useTheme();
  const [profitCheck, setProfitCheck] = useState(true);
  const [amendAlgoOrder, { isLoading: isPlacingOrder }] =
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      const response = await amendAlgoOrder({ ...formObj }).unwrap();
...
export default UpdateAlgoOrder;
