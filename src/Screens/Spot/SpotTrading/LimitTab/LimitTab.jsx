...
const LimitTab = ({ baseAsset, quoteAsset }) => {
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      if (buyProfitCheck) {
        profitLossFields["stopPrice"] = values?.tpTriggerPx;
        profitLossFields["type"] = "TAKE_PROFIT_LIMIT";
        
      }
      else if (buyLossCheck) {
        profitLossFields["stopPrice"] = values?.slTriggerPx;
        profitLossFields["type"] = "STOP_LOSS_LIMIT";
      } else {
  ...
  const onQuantityChange = () => {
    ...
    if (!!priceVal && !!quantityVal) {
      form.setFieldValue("total", convertIntoDecimal(quantityVal * priceVal));
      const availableBal = accountInfo?.data?.filter((item) => {
        return item.asset === quoteAsset ? item : "";
      });
      setBuySlider(
        convertIntoDecimal(
  ...
  const handleSellSubmit = async (values) => {
    try {
      ...
      // if (
      //   !availableBal?.[0]?.free ||
      //   Number(availableBal?.[0]?.free) < Number(values?.quantity)
      // ) {
      //   toast.error("Insufficient Balance!");
      //   return;
      // }
      ...
      if (sellProfitCheck) {
        profitLossFields["stopPrice"] = values?.tpTriggerPx;
        // profitLossFields["stopPrice"] = values?.tpOrdPx;
        profitLossFields["type"] = "TAKE_PROFIT_LIMIT";
        
      }
      else if (sellLossCheck) {
        profitLossFields["stopPrice"] = values?.slTriggerPx;
        // profitLossFields["stopPrice"] = values?.slOrdPx;
        profitLossFields["type"] = "STOP_LOSS_LIMIT";
      } else {
        profitLossFields["type"] = "LIMIT";
      };
  ...
  return (
    <TabContainer>
      ...
      >
        ...
        />
        <MaxSell
          balance={accountInfo?.data?.filter((item) => item?.asset === baseAsset)}
          asset={baseAsset}
        />
        <Divider style={{ margin: "10px 0" }} />
        <div className="checkbox-container">
